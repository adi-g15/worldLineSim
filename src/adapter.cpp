// there was a circular dependency b/w display.hpp and NodeAdapter.hpp, that's
// why this has been segregated
...
NodeAdapter::NodeAdapter(DisplayScreen dispScr, World_Node_Ptr world_node)
    : dispMngr(dispScr), node(world_node),
      Window(dispScr->multiverse_window,
             "World #.." + std::to_string(world_node->world_id() % 512)) {
    if (!node)
        throw std::runtime_error(
            "A non-null world node is required for adapter");

    this->set_layout(new GroupLayout());

    auto data_area = new Widget(this);
    auto layout =
        new GridLayout(Orientation::Horizontal, 2, Alignment::Middle, 15, 2);
    ...
    if (node->parent_node != nullptr) {
        ...
        new TextBox(data_area,
                    std::string("#") +
                        std::to_string(node->parent_node->world_id() % 512));
    }
    ...
    // auto diverge_btn = new ToolButton(button_group, FA_CODE_BRANCH,
    // "Diverge");
    ...
    (new Button(diverge_popup, "With current state", FA_AIR_FRESHENER))
        ->set_callback([&]() mutable { this->diverge_without_change(); });

    auto time_travel_btn =
        new PopupButton(diverge_popup, "TimeTravel", FA_HOURGLASS);
    time_travel_btn->set_callback(
        [&]() mutable { this->diverge_with_time_travel(); });
    time_travel_btn->set_tooltip(
        "In this you can edit the current world's state !!"
    ...
    popup_grid->set_layout(
        new GridLayout(Orientation::Horizontal, 2, Alignment::Middle, 4, 2));
    ...
    this->time_travel_create_btn =
        new Button(time_travel_entity_group, "Create !");
    ...
    remove_btn->set_callback([this]() mutable { this->remove(); });
...
void NodeAdapter::remove() { // since it's stopped, so no further calls to
...
void NodeAdapter::update() { // updates the content on the window, with updated
                             // content from the world_naode that is linked
    this->num_entity->set_value(
        static_cast<int>(this->node->world.entities.size()));
    ...
    static_cast<IntBox<statics::dimen_t> *>(dimensions[0])
        ->set_value(curr_dimen.mX);
    static_cast<IntBox<statics::dimen_t> *>(dimensions[1])
        ->set_value(curr_dimen.mY);
    static_cast<IntBox<statics::dimen_t> *>(dimensions[2])
        ->set_value(curr_dimen.mZ);
}
...
void NodeAdapter::diverge_with_time_travel() {
    ...
    if (this->time_travel_entity_widgets.size() >
        editable_state.entity_states.size()) {
    ...
    } else if (time_travel_entity_widgets.size() <
               editable_state.entity_states.size()) {
        ...
        for (; i < editable_state.entity_states.size(); i++) {
            ...
            time_travel_entity_widgets[i][0] = new Label(
                time_travel_entity_group,
                Entity_Type_Map[static_cast<int>(entity_state->entity_type)]);

            if ((entity_state->entity_type == Entity_Types::HUMAN) ||
                (entity_state->entity_type == Entity_Types::SNAKE) ||
                (entity_state->entity_type == Entity_Types::ROCK)) {
                time_travel_entity_widgets[i][1] =
                    new Widget(time_travel_entity_group);
                time_travel_entity_widgets[i][1]->set_layout(
                    new GridLayout(Orientation::Horizontal, 3));

                switch (entity_state->entity_type) {
                case Entity_Types::HUMAN: {
                    const auto &state =
                        static_cast<const HumanState *>(entity_state);

                    (time_travel_entity_widgets[i][1]->add<IntBox<dimen_t>>(
                         state->location.mX))
                        ->set_editable(true);
                    (time_travel_entity_widgets[i][1]->add<IntBox<dimen_t>>(
                         state->location.mY))
                ...
                case Entity_Types::SNAKE: {
                    const auto &state =
                        static_cast<const SnakeState *>(entity_state);
                ...
                case Entity_Types::ROCK: {
                    const auto &state =
                        static_cast<const RockState *>(entity_state);
                ...
                time_travel_entity_widgets[i][1] =
                    new Label(time_travel_entity_group, "No Location");
            }
        }
    }
    ...
    for (auto i = 0; i < editable_state.entity_states.size(); ++i) {
        ...
        case Entity_Types::HUMAN: {
            ...
            static_cast<IntBox<dimen_t> *>(
                time_travel_entity_widgets[i][1]->child_at(0))
                ->set_value(state->location.mX);
            static_cast<IntBox<dimen_t> *>(
                time_travel_entity_widgets[i][1]->child_at(1))
                ->set_value(state->location.mY);
            static_cast<IntBox<dimen_t> *>(
                time_travel_entity_widgets[i][1]->child_at(2))
                ->set_value(state->location.mZ);
