...
void Rabin::simulateExistence() {
    ...
    Graph_Box_3D<Box> *rand_box =
        this->parent_world
            ->world_plot[this->parent_world->world_plot.getRandomCoord()];
    ...
    while (this->is_jinda) {
        ...
        if (rand_box->get_coordinate().mX % 2 == 0)
            LOGGER::log_msg("Rabin marne se bach gaya");
        else
            LOGGER::log_msg("Rabin #{} bhaaga... from #{}", this->_id,
                            linear_iter.curr_box->get_coordinate());
#endif
...
void Rabin::pauseExistence() { is_jinda = false; }
