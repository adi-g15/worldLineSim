...
void Mathematician::simulateExistence() {
    ...
    while (should_research) {
        coord analyzed_location(std::rand() % 40, std::rand() % 40,
                                std::rand() % 40);
        shared_knowledge.push(analyzed_location); // join the civilisation :D

        if (std::rand() % 4 == 1) {
            LOGGER::log_msg(
                "Mathematician {} [#{}] has analyzed the coordinate: #{}...",
                this->name, this->_id, analyzed_location);
        } else if (std::rand() % 2 == 1) {
            LOGGER::log_msg("{} [Mathematician] embracing the sky's beauty",
                            this->name);
        } else {
