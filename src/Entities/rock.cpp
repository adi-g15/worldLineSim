...
Rock::Rock(World_Ptr world, const RockState& state): Rock(world, state.location) {}
...
Rock::Rock(World_Ptr world, const coord& location): Entity(Entity_Types::ROCK), location(world->world_plot[location], location) {
    ...
    LOGGER::log_trace("Rock #{} placed in world #{} at ({}, {}, {}}", this->_id, world->_id, this->location.point_coord.mX, this->location.point_coord.mY, this->location.point_coord.mZ);

	this->location.graph_box->getDataRef().addEntity(this);
}

RockState* Rock::_get_current_state() const
{
	return new RockState(this);
}
