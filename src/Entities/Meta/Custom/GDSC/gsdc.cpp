...
void Member::simulateExistence() {
	return;
	/*members remain with GDSC for now, abhi ke liye pure GDSC ko ghumne padega sorry :) */
	this->has_been_paused = false;

	while (this->should_wander && this->parent_world->is_world_running())
	{
		LOGGER::log_msg("[{} #{}] Refreshing factors...", mName, this->_id, this->curr_pos.point_coord);
...
void Member::pauseExistence() {
	...
	while (!this->has_been_paused)	{}	// blocking wait.... till it's stopped
}

// metrics === factors
void Member::refresh_metric() {
	auto &p = this->interactions[last_msg.substr(0,3)];
	p.second = p.second * fmod(
			(-0.2 + rand()),
			1
		) * p.first;

	// p.first ko yaha change nhi krenge, it's changed in interaction with mouth or ears
}
...
void Member::ears_listen(std::string sounds) {
	...
	// we had another interaction, update it... eventually we will refresh metrics
	this->interactions[sounds.substr(0,3)].first += sounds.size();
}

void Member::mouth_speak() {
	auto message_bytes = util::Random::random_bytes(
			ceil(this->talkative_factor * 15)
		);
	...
	auto message = std::string(
			...
			reinterpret_cast<char*>(message_bytes.data()) + message_bytes.size()
			);
...
GDSC::GDSC( World_Ptr const world, const std::string& full_name, const std::map< std::string, Member >& members ):
	Entity(Entity_Types::META),
	parent_world(world),
	group(members),
	full_name(full_name)
{
...
GDSC::GDSC( World_Ptr const world, const std::string& full_name, const std::string& members_details_file ):
	GDSC( world, full_name, _get_members_from_file(members_details_file) )
{}

std::map< std::string, Member > GDSC::_get_members_from_file(const std::string& members_details_file) {
...
void GDSC::broadcast(std::string msg) {
	...
	if ( wildcard_pos != std::string::npos ) { msg.erase( wildcard_pos ); }
...
void GDSC::pauseExistence() {
	pause_requested = false;
	LOGGER::log_msg("[GDSC {} #{}]: Phir milenge...", this->full_name, this->_id);
