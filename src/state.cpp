...
//State::~State()
//{
//	LOGGER::log_msg("Removing state for time {}", this->state_time);	// always removing the 1st state for some reason, couldn't know till now, what's causing for the destructor to be called, since it's stored simply as vector<State>, upon that why only the 1st element for the first 3 times atleast (after that invalid access error)
//	for (auto* state : entity_states)
//	{
//		delete state;
//	}
//	entity_states.clear();
