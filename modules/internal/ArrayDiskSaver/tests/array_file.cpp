...
struct serialisable {
	static inline char buffer[5000];	// a 512 Bytes buffer (use whatever max size an object may take)
	...
	std::pair<char*,u32> serialise() {
		...
		for (; i < std::size(buffer); i+=4)
		{
			...
			std::memcpy(reinterpret_cast<void*>(buffer + i), &tmp, sizeof(float));
		}
...
int main() {
	try {
		...
		std::cout << std::boolalpha << ref1.has_value() << " " << ref2.has_value() << std::endl;
