...
class db {
    ...
    static inline std::map<id_type, std::string_view>
        id_to_name_mapping; // used to save storage for multiple HumanState
                            // objects
  ...
  public:
    static void init() {
        ...
        source_path.erase(last_slash_index +
        ...
        if (name_reader) {
            ...
            female_names.emplace_back(
                "Human"); // general name, since failed to fetch names
        }
    }

    static const std::string &getRandomName(Gender gender, id_type id) {
        const id_type rand_index =
            rand() % (gender == Gender::MALE
                          ? male_names.size()
                          : female_names.size()); // random index
